...
def parse_segmentation_svg(svg_path: Path,
    ...
    print(f"Loaded {len(data.shapes)} shapes from {len(data.blocks)} blocks from {svg_path}")
    return data


def parse_combined_svg(svg_path: Path,
                       simplify_tolerance: float = DEFAULT_SIMPLIFY_TOLERANCE
                       ) -> SegmentationData:
    """Parse a combined SVG file generated by combine.py.

    This format differs from segmentation.svg:
    - Points are already in global coordinates (no transform needed)
    - Structure: <g id="plate-XXX"><g id="XXX-block-YYY"><polygon .../></g></g>
    - May contain <image> elements which are ignored
    ...
    Returns:
        SegmentationData with all shapes loaded (no block context since coords are global)
    """
    with open(svg_path, 'r') as f:
        content = f.read()
    ...
    # Extract SVG dimensions from root SVG
    svg_dims = re.search(r'<svg[^>]*width="([0-9.]+)"[^>]*height="([0-9.]+)"', content)
    if svg_dims:
        svg_width, svg_height = float(svg_dims.group(1)), float(svg_dims.group(2))
        print(f"Combined SVG dimensions: {svg_width:.0f}x{svg_height:.0f}")

    # Find all polygon elements directly (they have global coordinates)
    # Pattern matches: <polygon points="..." ... />
    polygon_pattern = re.compile(
        r'<polygon\s+[^>]*points="([^"]+)"[^>]*/?>',
        re.DOTALL
    )

    for match in polygon_pattern.finditer(content):
        ...
        if len(points) >= 3:
            if simplify_tolerance > 0:
                points = simplify_polygon(points, simplify_tolerance)
            # No block context since coordinates are already global
            data.shapes.append(Shape(points=points, block=None))

    print(f"Loaded {len(data.shapes)} shapes from combined SVG {svg_path}")
    return data
...
def process_segmentation(seg_path: Path, output_path: Path, use_plate_coords: bool = True,
    ...
    if not seg_data.shapes:
        print(f"No shapes found in: {seg_path}")
        return
...
Examples:
  ...
  uv run python generate_stl.py --manifest segmentation.json
    ...
    if args.segmentation:
        ...
        # Parse the combined SVG (uses different format than segmentation.svg)
        output_path = args.output or manifest_path.with_suffix(".stl")
        print(f"\nExtruding combined SVG to {output_path}...")

        seg_data = parse_combined_svg(combined_svg, args.simplify)
        ...
        for i, shape in enumerate(seg_data.shapes):
            # Combined SVG already has global coordinates, use points directly
            points = shape.points
